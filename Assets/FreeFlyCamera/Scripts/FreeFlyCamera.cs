...
public class FreeFlyCamera : MonoBehaviour
{
    ...
    private void SetCursorState()
    {
        ...
        // Only lock cursor when Shift is held down
        if (shiftHeld)
        {
            _wantedMode = CursorLockMode.Locked;
        }
    ...
    private void Update()
    {
        ...
        // Only process camera controls when Shift is held and cursor is locked
        if (Cursor.visible)
            return;

        bool shiftHeld = Input.GetKey(KeyCode.LeftShift) || Input.GetKey(KeyCode.RightShift);
        
        // Only allow camera movement when Shift is held down
        if (!shiftHeld)
            return;
